import {PaperClipIcon, XCircleIcon, PencilIcon} from "@heroicons/react/solid";
...
const EducationCard = (props) => {
	...
	const docRef = doc(db, "EducationCases", Id);
	const [interested, setInterested] = useState(false);
	useEffect(() => {
		if (user) {
			setInterested(
				user.favoritePosts.educationPosts.find((item) => item === Id)
			);
		}
	}, [post, user]);
	// Interested Content
	...
	const interestedClick = async () => {
		...
		if (interested) {
			const filteredPosts = user.favoritePosts.educationPosts.filter(
				(item) => item !== Id
			);
			await updateDoc(userDocRef, {
				favoritePosts: {
					...user.favoritePosts,
					educationPosts: filteredPosts,
				},
			});
		} else {
			const newFavoritePosts = [...user.favoritePosts.educationPosts, Id];
			await updateDoc(userDocRef, {
				favoritePosts: {
					...user.favoritePosts,
					educationPosts: newFavoritePosts,
				},
			});
		}
	};
			...
			<div className="bg-white  overflow-hidden sm:rounded-lg border ">
				<div className="border-t border-gray-200 px-4 py-5 sm:p-0">
					<dl className="sm:divide-y sm:divide-gray-200">
						<div className="py-4 lg:grid-cols-3 sm:py-5 sm:grid sm:grid-cols-4 sm:gap-4 sm:px-6">
						...
						<div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-3 sm:px-6">
						...
						<div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
							<dt className="text-sm font-medium text-gray-500">Case Info:</dt>
							<dd className=" flex  justify-end mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
								{Info}
							</dd>
						</div>
