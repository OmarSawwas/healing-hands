				...
				await onSnapshot(userRef, async (userDoc) => {
					...
					workPosts.map(async (item) => {
						...
						if (workDocs.length !== 0) {
							setWorkDocs((prevState) => [
								prevState,
								{id: post.id, ...post.data()},
							]);
						} else {
							setWorkDocs([{id: post.id, ...post.data()}]);
						}
					});
				});
			}
				...
				await onSnapshot(userRef, async (userDoc) => {
					...
					educationPosts.map(async (item) => {
						...
						if (educationDocs.length !== 0) {
							setEducationDocs((prevState) => [
								prevState,
								{id: post.id, ...post.data()},
							]);
						} else {
							setEducationDocs([{id: post.id, ...post.data()}]);
						}
					});
				});
			}
			if (user.favoritePosts.animalPosts) {
				await onSnapshot(userRef, async (userDoc) => {
					...
					animalPosts.map(async (item) => {
						...
						const post = await getDoc(postRef);
						if (animalDocs.length !== 0) {
							setAnimalDocs((prevState) => [
								prevState,
								{id: post.id, ...post.data()},
							]);
						} else {
							setAnimalDocs([{id: post.id, ...post.data()}]);
						}
					});
				});
			}
